...
<div id="news-headers-section"  class="news-container news-header2">
...
const getNews2Container = function (searchInput) {
  ...
  fetch(url)
    ...
    .then((data) => {
      console.log(data);
      console.log(`${data.response.results[10].fields.thumbnail}`);
      var date1 = new Date(
...
const getNews5Article2 = function (searchInput) {
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      <p class="news5-article2-date webpage-header">${data.response.results[33].webPublicationDate}</p>
      </a>
      `;
    });
};
