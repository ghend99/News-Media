...
const getNews1Article1 = function (searchInput) {
  let url = `https://content.guardianapis.com/search?section=${searchInput}&page-size=30&show-blocks=all&api-key=${apiKey}&show-fields=thumbnail&show-fields=all`;
  fetch(url)
    .then((res) => res.json())
    .then((data) => {
      console.log(data);
      news1Article1Container.innerHTML = `
        <a href="${data.response.results[0].webUrl}" target="_blank">
